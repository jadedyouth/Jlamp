...
if (cnt == 25) { md = random(0,3);  //when you reach a set number randomly pick a mode
while (cnt < 29) { cnt ++; // keep counting and while that number is below another number
...
void setup() {
FastLED.addLeds<APA102,3,13,BGR>(leds, LED_COUNT); //initalise strips
pinMode(12, INPUT);  //in case we want to use an external button
